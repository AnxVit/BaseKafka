...
func getIDFromRequest(formValue string, ctx *gin.Context) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf(
			"failed parses ID from form value %s: %w", formValue, err)
	}
	return id, nil
}
...
func sendKafkaMessage(producer sarama.SyncProducer,
	...
	if err != nil {
		return fmt.Errorf("failed to marshal notificaton: %w", err)
	}
...
func main() {
	...
	fmt.Printf("Kafka PRODUCER started at http://localhosts%s\n", ProducerPort)
